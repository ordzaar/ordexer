 ...
 */
export class OrdexerServerApp<App extends NestFastifyApplication = NestFastifyApplication> {
  protected app?: App;
  ...
  /**
   * Start listening on APP_PORT with APP_HOSTNAME, the default being 0.0.0.0:5742
   */
  async start(): Promise<App> {
    const app = await this.init();

    const config = app.get(ConfigService);
    const port = config.get<number>("APP_PORT", 5742);
    ...
    logger.log(`======================= ORDEXER SERVER STARTED (v${serverVersion}) =======================`);
